uniform highp float u_ligthPowers;
...
void main(void)
{
    ...
    vec4 diffColor = diffMatColor * u_ligthPowers * max(0.0, dot(v_normal, -lightVect)) / (1.0 + 0.25 * pow(len,2.0));
    resultColor += diffColor;

    vec4 ambientColor = ambientFactor * diffMatColor;
    resultColor += ambientColor;

    vec4 specularColor = vec4(1.0,1.0,1.0,1.0) * u_ligthPowers * pow(max(0.0, dot(reflectLight, -eyeVect)),specularFactor) / (1.0 + 0.25 * pow(len,2.0));
